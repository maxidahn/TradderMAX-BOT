...
def build_dashboard(executor):
    ...
    # ── Protección opcional del dashboard (para cuando es público en la nube) ──
    # Si V2_DASHBOARD_TOKEN está seteado, hay que entrar con  ?token=EL_TOKEN
    DASH_TOKEN = os.getenv("V2_DASHBOARD_TOKEN", "")
...
def main():
    ...
    host = os.getenv("V2_HOST", "0.0.0.0")   # 0.0.0.0 = accesible en la nube; local igual anda por localhost
    logger.info("📊 Dashboard escuchando en %s:%d", host, config.port)
